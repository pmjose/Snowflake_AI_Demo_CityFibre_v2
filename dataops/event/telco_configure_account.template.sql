...
ALTER SESSION SET QUERY_TAG = '''{"origin":"sf_sit-is", "name":"CityFibre UK AI Demo", "version":{"major":1, "minor":0},"attributes":{"is_quickstart":0, "source":"sql"}}''';

-- ============================================================================
-- Create Warehouse Early (required for EXECUTE IMMEDIATE blocks)
-- ============================================================================
CREATE WAREHOUSE IF NOT EXISTS {{ env.EVENT_WAREHOUSE | default('CITYFIBRE_UK_DEMO_WH') }}
...
GRANT USAGE ON WAREHOUSE {{ env.EVENT_WAREHOUSE | default('CITYFIBRE_UK_DEMO_WH') }} TO ROLE {{ env.EVENT_ATTENDEE_ROLE | default('TELCO_ANALYST_ROLE') }};
GRANT OPERATE ON WAREHOUSE {{ env.EVENT_WAREHOUSE | default('CITYFIBRE_UK_DEMO_WH') }} TO ROLE {{ env.EVENT_ATTENDEE_ROLE | default('TELCO_ANALYST_ROLE') }};
...
CREATE DATABASE IF NOT EXISTS {{ env.EVENT_DATABASE | default('CITYFIBRE_UK_AI_DEMO') }}
    COMMENT = 'Database for CityFibre UK AI demo - All stages and data pre-loaded';

USE DATABASE {{ env.EVENT_DATABASE | default('CITYFIBRE_UK_AI_DEMO') }};

-- Create schemas
CREATE OR REPLACE SCHEMA {{ env.EVENT_SCHEMA | default('CITYFIBRE_UK_SCHEMA') }}
...
CREATE OR REPLACE NETWORK RULE telco_web_access_rule
  ...
  COMMENT = 'Permissive network access for CityFibre UK demo web scraping';

CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION cityfibre_uk_external_access_integration
  ALLOWED_NETWORK_RULES = (telco_web_access_rule)
  ENABLED = TRUE
  COMMENT = 'External access for web scraping and API calls';

GRANT USAGE ON INTEGRATION cityfibre_uk_external_access_integration TO ROLE {{ env.EVENT_ATTENDEE_ROLE | default('TELCO_ANALYST_ROLE') }};

-- ============================================================================
-- Step 8: Create Email Notification Integration
-- ============================================================================

CREATE OR REPLACE NOTIFICATION INTEGRATION cityfibre_uk_email_int
  TYPE = EMAIL
  ENABLED = TRUE
  COMMENT = 'Email integration for sending notifications from Intelligence Agent';

GRANT USAGE ON INTEGRATION cityfibre_uk_email_int TO ROLE {{ env.EVENT_ATTENDEE_ROLE | default('TELCO_ANALYST_ROLE') }};
