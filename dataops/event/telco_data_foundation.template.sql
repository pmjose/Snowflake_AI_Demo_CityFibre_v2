-- ============================================================================
-- CityFibre UK AI Demo - Data Foundation (DataOps Template)
-- ============================================================================
-- Description: Creates tables and loads CityFibre UK fibre/entertainment data
...
) COMMENT = 'Product categories for CityFibre UK broadband, TV, mobile, and business services';
...
CREATE OR REPLACE TABLE product_dim (
    ...
    vertical VARCHAR(50)
) COMMENT = 'CityFibre UK services: broadband, TV, mobile, WiFi guarantee, and business connectivity';
...
CREATE OR REPLACE TABLE vendor_dim (
    ...
    state VARCHAR(10),
    zip VARCHAR(20)
) COMMENT = 'CityFibre UK suppliers and partners: network build, CPE/WiFi, cloud, and content';
...
) COMMENT = 'CityFibre UK organizational departments';

-- Region Dimension
CREATE OR REPLACE TABLE region_dim (
    region_key INT PRIMARY KEY,
    region_name VARCHAR(100) NOT NULL
) COMMENT = 'UK regions: London & South East, Midlands, North West, Yorkshire & Humber, North East, Scotland, Wales, South West, and national backbone';
...
CREATE OR REPLACE TABLE sales_rep_dim (
    ...
    hire_date DATE
) COMMENT = 'CityFibre UK sales representatives and channel account managers';
...
CREATE OR REPLACE TABLE employee_dim (
    ...
    gender VARCHAR(1),
    hire_date DATE
) COMMENT = 'CityFibre UK employees for HR analysis';
...
CREATE OR REPLACE TABLE job_dim (
    ...
    job_level INT
) COMMENT = 'Job titles and levels within CityFibre UK';

-- Location Dimension (HR)
CREATE OR REPLACE TABLE location_dim (
    location_key INT PRIMARY KEY,
    location_name VARCHAR(200) NOT NULL
) COMMENT = 'CityFibre UK offices, network hubs, and data centres';

-- ============================================================================
-- Step 3: Create Fact Tables
-- ============================================================================

-- CityFibre UK KPI Reference (demo)
CREATE OR REPLACE TABLE cityfibre_kpi (
    metric VARCHAR(200) PRIMARY KEY,
    value NUMBER(18,2),
    as_of_note VARCHAR(200),
    category VARCHAR(50)
) COMMENT = 'Reference KPIs for CityFibre UK demo (RFS, take-up, financing, speed)';
...
) COMMENT = 'Illustrative ARPA/ARPU by segment for CityFibre UK demo (values in GBP, column names retained for compatibility)';
...
CREATE OR REPLACE TABLE b2c_customers (
    ...
    status VARCHAR(50),
    region_key INT
) COMMENT = 'CityFibre UK B2C broadband/TV/mobile customers (demo)';
...
CREATE OR REPLACE TABLE b2c_subscriptions (
    ...
    mobile_sims INT,
    wifi_guarantee BOOLEAN
) COMMENT = 'CityFibre UK B2C subscriptions with bundle details (demo)';
...
CREATE OR REPLACE TABLE city_subs (
    ...
    premises_rfs NUMBER(18,0),
    takeup_pct NUMBER(6,3)
) COMMENT = 'City-level subs, premises RFS, and take-up for broadband, TV, voice, and mobile';

-- Postcode-sector RFS/take-up (demo)
CREATE OR REPLACE TABLE city_sector_rfs (
    city VARCHAR(100),
    ...
    avg_install_days INT,
    data_as_of VARCHAR(50)
) COMMENT = 'Postcode-sector ready-for-service and take-up by city';

-- ISP tariffs (CityFibre footprint)
CREATE OR REPLACE TABLE isp_tariffs (
    isp VARCHAR(100),
    product VARCHAR(150),
    speed_mbps NUMBER(10,0),
    standard_fee_gbp NUMBER(10,2),
    promo_fee_gbp NUMBER(10,2),
    promo_months INT,
    contract_months INT,
    cityfibre_footprint BOOLEAN,
    notes VARCHAR(200)
) COMMENT = 'Current ISP tariffs on CityFibre footprint with promo and contract detail';
...
-- Load CityFibre UK KPI Reference
...
-- Load Postcode-sector RFS
COPY INTO city_sector_rfs
FROM @{{ env.EVENT_DATA_STAGE | default('DATA_STAGE') }}/demo_data/city_sector_rfs.csv
FILE_FORMAT = CSV_FORMAT
ON_ERROR = 'CONTINUE';

-- Load ISP tariffs
COPY INTO isp_tariffs
FROM @{{ env.EVENT_DATA_STAGE | default('DATA_STAGE') }}/demo_data/isp_tariffs.csv
FILE_FORMAT = CSV_FORMAT
ON_ERROR = 'CONTINUE';
...
UNION ALL
SELECT '', 'cityfibre_kpi', COUNT(*) FROM cityfibre_kpi
UNION ALL
...
-- Show all tables
SHOW TABLES IN SCHEMA {{ env.EVENT_SCHEMA | default('CITYFIBRE_UK_SCHEMA') }};

SELECT 'CityFibre UK AI Demo data foundation complete!' AS status,
       '{{ env.EVENT_DATABASE | default("CITYFIBRE_UK_AI_DEMO") }}' AS database_name,
